...
if jq -e 'has("supply_chain")' "$manifest" >/dev/null; then
  ...
  require '(.supply_chain.profile == "basic") or ((.supply_chain.evidence | type == "object") and ((.supply_chain.evidence.sbom_paths // []) | length > 0) and ((.supply_chain.evidence.build_record_paths // []) | length > 0))' "standard and high-trust supply-chain profiles require SBOM and build evidence paths"
fi
