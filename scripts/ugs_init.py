...
def files(source, profile, with_document_map=False):
    ...
    output = {
        ...
        ".ugs/schema/cr.schema.json": ((source / ".ugs/schema/cr.schema.json") if (source / ".ugs/schema/cr.schema.json").exists() else (source / "bootstrap/templates/cr.schema.json")).read_text(),
        ".ugs/schema/cr-attestation.schema.json": ((source / ".ugs/schema/cr-attestation.schema.json") if (source / ".ugs/schema/cr-attestation.schema.json").exists() else (source / "bootstrap/templates/cr-attestation.schema.json")).read_text(),
        ...
        "cr/README.md": (source / "bootstrap/templates/cr/README.md").read_text(),
        "cr/TEMPLATE.md": (source / "bootstrap/templates/cr/TEMPLATE.md").read_text(),
        ...
        "scripts/cr_model.py": (source / "scripts/cr_model.py").read_text(),
        "scripts/cr_attestation.py": (source / "scripts/cr_attestation.py").read_text(),
        "scripts/validate_cr_review.sh": (source / "scripts/validate_cr_review.sh").read_text(),
        "scripts/validate_cr_attestation.sh": (source / "scripts/validate_cr_attestation.sh").read_text(),
